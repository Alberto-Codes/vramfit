...
`imatrix_zero_count_experts`, as `(stack, expert)` pairs — a
report, never a gate. A matrix the reader cannot vouch for halts
the pack before the quantizer runs: not an imatrix, no counts, an
unknown tensor suffix, a sums tensor without its counts twin, a
count that is negative or not finite, or a count length that
contradicts the base tensor. The read needs gguf-py, which the
pack extra provisions — a matrix-less pack touches neither the
library nor the file.
