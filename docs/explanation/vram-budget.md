...
arithmetic is the entire reason the project exists — and it's also why the
benchmark's serving path runs through GGUF rather than vLLM's 4-bit kernel
floor, per [ADR-0010](../adr/0010-sub-4-bit-serving-path.md).
