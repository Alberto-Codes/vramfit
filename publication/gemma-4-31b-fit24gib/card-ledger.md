...
`analysis/psai-gui-kv9.json`, generated by
`run-462-full/psai-analysis-462.py` from the run archive under
`run-462-full/results/`, `run-463-qat-regen/`, and `run-462-4090/`.
...
| Ratios 1.18x (content mean), 3.6x (content median), 4.5x (all-position mean), markup 1.13 / 0.03, pos0 1.61 / 0.11 | recomputed from the artifact's class tables | card prose |
| Content mean 0.0045 above (vision bound) | `campaign-breakdown.json`, the vision-bound ledger rows | card prose, cross-reference |
...
| H100 throughput 66.1 / 75.9 / 39.4 tok/s over 1,349 gens per arm | `probe-*.gen.jsonl` server timings, [#462 throughput comment](https://github.com/Alberto-Codes/vramfit/issues/462#issuecomment-5503783237) | artifact `results.throughput.h100_sxm_80gb_cuda` |
| 4090 throughput 47.8 / 43.3 / 47.7 tok/s, sd 0.5 / 0.2, mean 20 tokens against the 48 cap, 20 gens per arm | `run-462-4090/probe-4090-*.gen.jsonl`, `stage-times.txt`, `server-4090-*.log`, #462 facts comment | artifact `results.throughput.rtx4090_24gib_vulkan` |
...
| QAT 512-token regeneration, 4,047 verdicts, judge `claude-haiku-4-5` via `claude -p` | #462 ruling record, `run-463-qat-regen/` | artifact `method.accuracy` |
