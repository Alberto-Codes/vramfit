...
uv run python scripts/check_ty_overrides.py  # lazy optional imports carry a ty override
...
- `hooks/pr_guard.py` — a PreToolUse guard on `gh pr ready`,
  ...
  because the guard cannot verify a claim that a search ran.
  The guard reminds rather than enforces, and it fails open.
