...
Examples:
    ...
    ```python
    from quantfit.adapters.inbound.cli_options import parse_gpu_memory

    assert parse_gpu_memory("1GiB", "auto") == 2**30
    ```

See Also:
    - [quantfit.adapters.inbound.cli_scan][]: The scan command.
    - [quantfit.adapters.inbound.cli_validate][]: The validate command.
"""
...
def check_imatrix(imatrix: Path | None, method: str) -> None:
    ...
    if method != "kquant":
        raise typer.BadParameter(
            ...
            "RTN has no weighted C counterpart"
        )
