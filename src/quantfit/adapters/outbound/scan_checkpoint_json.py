...
instead of mixing two scans' numbers. Writes go through the same
atomic-replace path as the artifact adapters, so a crash mid-write
...
class JsonScanCheckpointFile:
    ...
    def append(self, fingerprint: str, measurement: Measurement) -> None:
        ...
        The whole checkpoint is rewritten atomically. Checkpoints stay
        small (one entry per grid cell), so the rewrite cost is noise
        next to a measurement's calibration pass.
...
def _parse_checkpoint(data: object, fingerprint: str) -> tuple[Measurement, ...]:
    ...
    Raises:
        ArtifactError: If a field is missing or mistyped, a damage value
            is invalid, or the stored fingerprint differs.
    """
    root = _get_dict(data, "$")
