...
``quantfit_schema`` envelope (version 1). Writer only — the reader
path for card tooling is deferred (issue #65). Absent tiers
serialize as JSON null, as do the toolchain's tier-3 fields, so
every schema-1 sidecar carries the same key set. The domain types
(`quantfit.domain.evals`) enforce the value invariants before
...
See Also:
    - [quantfit.ports.outbound][]: `EvalsSidecarSink`, which
...
from quantfit.adapters.outbound.json_common import _save_json
...
# and nothing else's.
EVALS_SIDECAR_SCHEMA_VERSION: Final[int] = 1
...
def sidecar_to_dict(sidecar: EvalsSidecar) -> dict[str, Any]:
    ...
    tier-3 fields — the key set never varies within schema 1.
    ...
    """
    return {
        "quantfit_schema": EVALS_SIDECAR_SCHEMA_VERSION,
        "artifact": {
