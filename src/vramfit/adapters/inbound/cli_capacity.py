...
Reads a packed recipe, subtracts its predicted weight bytes and the
runtime overhead from the card, and reports what the remaining KV
headroom buys (#422): the largest context, the sequence count at a
fixed ``--context``, and an image capacity at a caller-supplied
``--tokens-per-image`` cost. The solvers live in
...
def capacity(
    ...
    tokens_per_image: Annotated[
        ...
        typer.Option(
            min=1,
            help="Ruled image token cost — adds the image-capacity line. "
            "vramfit rules no vision policy (#236, #419).",
        ),
    ] = None,
    overhead: Annotated[
    ...
    Raises:
        typer.BadParameter: If both or neither shape source is
            given, a size/dtype option is malformed, or an integer
