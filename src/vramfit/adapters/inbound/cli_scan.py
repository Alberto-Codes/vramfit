...
def _build_meter(
    ...
    gpu_memory: int | None,
    within_group: Literal["rtn", "kquant", "q0", "q0-imx2", "q0-fit2"] = "rtn",
...
def scan(
    ...
    within_group: Annotated[
        ...
        typer.Option(
            ...
            "(precisions 8/4/2), q0-imx2 for assisted 2-bit, or "
            "q0-fit2 for the same 2-bit encoder without a matrix."
        ),
    ] = "rtn",
    imatrix: Annotated[
        Path | None,
        typer.Option(
            ...
            "q0-imx2 requires this file, and q0-fit2 refuses it. "
    ...
    The command reads the calibration file's SHA-256 and byte count
    before the model loads, records both in the map, and folds both
    into the fingerprint. So a corpus re-issued behind an unchanged
    ``--calibration`` path refuses the old checkpoint instead of
    ...
    ``--within-group`` takes ``rtn``, ``kquant``, ``q0``,
    ``q0-imx2``, or ``q0-fit2``, and it selects the quantization
    the meter applies
    ...
    assisted 2-bit fit, and ``q0-fit2`` runs that same 2-bit
    encoder at weight 1.0 with no matrix at all. Each pairs only
    ...
    q0-imx2 requires that file and assists nominal 2, and
    q0-fit2 refuses it. Under q0,
    ...
    parsed_within_group, method_token = parse_within_group(
