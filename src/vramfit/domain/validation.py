...
[vramfit.adapters.inbound.cli_validate][].
...
See Also:
    - [vramfit.domain.model][]: `Recipe`, the prediction's source.
    - [vramfit.ports.outbound][]: `DamageMeter.measure_recipe`, the
      measurement side.
"""
...
class ValidationResult:
    ...
    Examples:
        ...
        ```python
        from vramfit.domain.validation import ValidationResult
...
def summed_marginal_damage(recipe: Recipe) -> float:
    ...
    Examples:
        ...
        ```python
        from vramfit.domain.validation import summed_marginal_damage
...
def validation_result(recipe: Recipe, measured_damage: float) -> ValidationResult:
    ...
    Args:
        ...
        measured_damage: Whole-recipe damage from the
            `vramfit.ports.outbound.DamageMeter` port's
            ``measure_recipe``.
