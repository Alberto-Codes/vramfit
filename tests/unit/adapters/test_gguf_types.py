...
def test_type_table_covers_the_adr_0010_candidate_set() -> None:
    assert set(GGML_TYPE_BY_BITS) == {8, 4, 3, 2}
    assert set(BASE_FTYPE_BY_BITS) == {8, 4, 3, 2}


@pytest.mark.parametrize(
    ("bits", "expected"),
    [(8, "q8_0"), (4, "q4_k"), (3, "q3_k"), (2, "q2_k")],
    ids=["8-bit", "4-bit", "3-bit", "2-bit"],
)
...
def test_ggml_type_for_unknown_bits_raises_pack_error() -> None:
    with pytest.raises(PackError, match="no GGUF type maps 6-bit"):
        ggml_type_for(6)
...
def test_base_type_with_unmapped_floor_raises_pack_error() -> None:
    recipe = make_recipe(("model.layers.0", 8), ("model.layers.1", 5))

    with pytest.raises(PackError, match="no GGUF base type maps 5-bit"):
        base_type(recipe)
